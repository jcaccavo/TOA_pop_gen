...
colorss = c("#64B7CC",  "#470E65", "#194B32", "#B3093F", "#F27607", "#70B987", "#8000FFB3","#FF00c5", "#ffe002", "#323232")
#colorss = c("#FC7C0C", "#117733", "#B20000", "#332288", "#64B7CC",  "#470E65", "#194B32")
#colorss = c("cyan2", "goldenrod", "gray3", "firebrick4", "darkslategray", "darkblue", "chartreuse2")
...
# Plot minK
# bp<-barplot(t(as.matrix(tbl[[1]][order(labels$n),])), col=rainbow(n=minK, alpha = 0.7),xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces)
bp<-barplot(t(as.matrix(tbl[[1]][order(labels$n),])), col=colorss,xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces)
axis(3,at=bp,labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.6)
# Plot higher K values
# if(maxK>minK)lapply(2:(maxK-1), function(x) barplot(t(as.matrix(tbl[[x]][order(labels$n),])), col=rainbow(n=x+1, alpha = 0.7),xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces))
if(maxK>minK)lapply(2:(maxK-1), function(x) barplot(t(as.matrix(tbl[[x]][order(labels$n),])), col=colorss,xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces))
